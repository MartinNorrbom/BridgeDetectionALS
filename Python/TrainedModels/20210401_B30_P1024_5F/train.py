...
def train_one_epoch(sess, ops, train_writer):
    ...
    for fn in range(len(TRAIN_FILES)):
        ...
        current_data, current_label, current_label_seg = accessDataFiles.load_h5_F5(TRAIN_FILES[fn])
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for fn in range(len(TEST_FILES)):
        ...
        current_data, current_label, current_label_seg = accessDataFiles.load_h5_F5(TEST_FILES[fn])
        # Get points for the input layer.
        current_data = current_data[:,0:NUM_POINT,:]
        current_label_seg = np.squeeze(current_label_seg)
