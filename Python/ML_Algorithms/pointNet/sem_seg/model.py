...
def placeholder_inputs(batch_size, num_point):
    pointclouds_pl = tf.placeholder(tf.float32,
                                     shape=(batch_size, num_point, 3)) # Changed from 9 to 3
...
def get_model(point_cloud, is_training, bn_decay=None):
    ...
    # CONV
    net = tf_util.conv2d(input_image, 64, [1,3], padding='VALID', stride=[1,1], # Changed from 9 to 3
    ...
    net = tf_util.conv2d(net, 2, [1,1], padding='VALID', stride=[1,1], # Changed from 13 to 2
...
if __name__ == "__main__":
    with tf.Graph().as_default():
        a = tf.placeholder(tf.float32, shape=(32,1024,3)) # Changed from 9 to 3 and 4096 to 1024
        ...
        with tf.Session() as sess:
            ...
            for i in range(100):
                print(i)
                sess.run(net, feed_dict={a:np.random.rand(32,1024,3)}) # Changed from 9 to 3 and 4096 to 1024
